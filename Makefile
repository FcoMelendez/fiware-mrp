...
start-emulator:
	$(COMPOSE) down -v --remove-orphans
	EMULATOR_MODE=live $(COMPOSE) up -d --build mongo orion-ld context-server mrp-api inventory-service bom-service manufacturing-service scheduler-service shopfloor-service emulator-gateway emulator-ui
...
test-all:
	...
	$(COMPOSE) down -v --remove-orphans
	$(COMPOSE) up -d mongo orion-ld context-server mrp-api inventory-service bom-service manufacturing-service scheduler-service shopfloor-service
	./scripts/wait-for-orion.sh
